...
config override and map selection via `OS.GetCmdlineUserArgs()` (`maze_bots/player/
controllers/RLConfig.cs`, `maze_bots/maps/MapSelection.cs`), which only sees args
placed after Godot's own `--` separator — so the stock kwargs mechanism can never
reach them. Importing this module patches in a `GodotEnv` subclass (used by
`StableBaselinesGodotEnv`) that inserts that separator when an `rl_config` or
`map_name` kwarg is present.
"""
...
def build_launch_cmd(
    ...
    **kwargs: object,
    ...
    verbatim. Both are appended after a `--` separator so maze_bots picks them up via
